...
Vec3 BxdfGGX::phase(float _roughness, float _alpha_x, float _alpha_y, const Surface &surface, const Vec3 &dir_in, const Vec3 &dir_out, const Vec3 &norm) {
    ...
    auto d = [&](const Vec3 &w) {
        ...
        float cos2_theta = cos_theta * cos_theta;
        float sin2_theta = 1.0f - cos2_theta;
        float ax2 = _alpha_x * _alpha_x;
        float ay2 = _alpha_y * _alpha_y;
        float s = cos2_theta + sin2_theta * (cos2_phi / ax2 + sin2_phi / ay2);
        return INV_PI / (_alpha_x * _alpha_y * s * s);
    };
    ...
    auto lambda = [&](const Vec3 &w) {
        ...
        float cos2_phi = cos_phi * cos_phi;
        float sin2_phi = 1.0f - cos2_phi;
        float alpha2 = _alpha_x * _alpha_x * cos2_phi + _alpha_y * _alpha_y * sin2_phi;
        return (std::sqrt(1.0f + alpha2 * tan2_theta) - 1.0f) * 0.5f;
    };
    ...
    // modified from disney brdf
    float fd90 = 0.5f + 2.0f * surface.cos_theta(half) * _roughness;
    float diffuse =
        INV_PI *
        (1.0f + (fd90 - 1.0f) * x5(1.0f - surface.cos_theta(dir_in))) *
        (1.0f + (fd90 - 1.0f) * x5(1.0f - surface.cos_theta(dir_out))) * _roughness;
